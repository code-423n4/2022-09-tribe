...
async function main() {
  ...
  if (!enableForking) {
    throw new Error('Please provide the url for a mainnet node / endpoint here.');
    provider = new ethers.providers.JsonRpcProvider('fixme');
  } else {
    throw new Error('Please provide the url for an instance of anvil forked from mainnet here.');
    provider = new ethers.providers.JsonRpcProvider('fixme');
  }
